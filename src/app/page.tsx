  ...
  return (
    <main className="min-h-screen bg-amber-50 text-slate-900">
      ...
      <div className="flex w-full max-w-[1400px] mx-auto flex-col gap-2 px-2 py-3 sm:px-8 sm:py-4">
        ...
        <aside className="chat-panel relative z-10 h-[150px] w-full shrink-0 md:sticky md:top-4 md:h-[430px] md:w-80"><div className="flex items-center justify-between border-b border-amber-200 pb-2"><div><h2 className="section-title">指導チャット</h2><p className="text-xs text-slate-500">全端末にリアルタイム同期</p></div><span className="live-dot">LIVE</span></div><div className="chat-list max-h-[120px] overflow-y-auto md:max-h-[250px]">{messages.length === 0 ? <p className="py-4 text-center text-sm text-slate-400">牌譜を見ながら会話できます</p> : messages.map((message) => <div key={message.id} className={`chat-bubble ${message.role === role ? "mine" : ""}`}><div className="flex justify-between gap-2 text-[11px] font-bold text-slate-500"><span>{message.role === "coach" ? "指導者" : "打者"}</span><time>{message.time}</time></div><p className="mt-1 text-sm">{message.text}</p></div>)}</div><form onSubmit={sendChat} className="mt-auto hidden gap-2 border-t border-amber-200 pt-2 md:flex"><input value={chatText} onChange={(event) => setChatText(event.target.value)} placeholder="メッセージを入力" className="chat-input" /><button type="submit" className="send-button" aria-label="送信">送信</button></form></aside>
        </div>
