  ...
  return (
    <main className="min-h-screen bg-amber-50 text-slate-900">
      ...
      <div className="flex w-full max-w-[1400px] mx-auto flex-col gap-2 px-2 py-3 sm:px-8 sm:py-4">
        ...
        <aside className="chat-panel relative z-10 h-[180px] w-full shrink-0 md:sticky md:top-4 md:h-[430px] md:w-80"><div className="flex items-center justify-between border-b border-amber-200 pb-2"><div><h2 className="section-title">指導チャット</h2><p className="text-xs text-slate-500">全端末にリアルタイム同期</p></div><span className="live-dot">LIVE</span></div><div className="chat-list max-h-[100px] overflow-y-auto md:max-h-[250px]">{messages.length === 0 ? <p className="py-4 text-center text-sm text-slate-400">牌譜を見ながら会話できます</p> : messages.map((message) => <div key={message.id} className={`chat-bubble ${message.role === role ? "mine" : ""}`}><div className="flex justify-between gap-2 text-[11px] font-bold text-slate-500"><span>{message.role === "coach" ? "指導者" : "打者"}</span><time>{message.time}</time></div><p className="mt-1 text-sm">{message.text}</p></div>)}</div><form onSubmit={sendChat} className="mt-auto flex gap-2 border-t border-amber-200 pt-2"><input value={chatText} onChange={(event) => setChatText(event.target.value)} placeholder="メッセージを入力" className="chat-input" /><button type="submit" className="send-button" aria-label="送信">送信</button></form></aside>
        </div>
        <div className="table-surface sticky bottom-0 z-20 w-full md:static md:z-10"><div className="flex items-center justify-between"><div><h2 className="section-title">あなたの手牌</h2><p className="text-xs text-slate-500">東家 / {board.hands[0].length}枚</p></div><span className={`turn-pill ${board.phase === "player" && role === "player" ? "turn-pill-active" : ""}`}>{role === "coach" ? "観戦中" : board.phase === "player" ? "あなたの番" : "CPU進行"}</span></div><div className="hand-row justify-center">{board.hands[0].map((tile, index) => <TileCard key={tile.id} tile={tile} onClick={() => discard(index)} disabled={!isHost || role !== "player" || board.gameMode !== "PLAYING" || board.phase !== "player" || board.turn !== 0 || isProcessingCpu} className={board.hands[0].length === 14 && index === 13 ? "ml-3" : ""} />)}</div></div>
      </div>
    </main>
  );
}