...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-amber-50 text-slate-900">
      <header className="mobile-compact-header border-b border-amber-200 bg-white/75 px-4 py-4 shadow-sm backdrop-blur sm:px-8"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-amber-700">MAHJONG COACHING ROOM</p><h1 className="mt-1 text-2xl font-black tracking-tight sm:text-3xl">東一局 / 指導対局</h1></div><div className="text-right"><p className="text-xs text-slate-500">{connection}</p><p className="mt-1 text-sm font-bold text-amber-800">残り {board.wall.length} 枚</p></div></div></header>
      <div className="flex w-full max-w-[1400px] mx-auto flex-col gap-2 px-2 py-3 pb-48 sm:px-8 sm:py-4 md:pb-0">
        ...
        <div className="table-surface fixed inset-x-0 bottom-0 z-50 w-full max-w-full overflow-hidden rounded-none border-x-0 md:static md:z-10 md:rounded-lg md:border-x"><div className="flex items-center justify-between"><div><h2 className="section-title">あなたの手牌</h2><p className="text-xs text-slate-500">東家 / {board.hands[0].length}枚</p></div><span className={`turn-pill ${board.phase === "player" && role === "player" ? "turn-pill-active" : ""}`}>{role === "coach" ? "観戦中" : board.phase === "player" ? "あなたの番" : "CPU進行"}</span></div><div className="hand-row hand-row-mobile flex w-full max-w-full flex-nowrap gap-0.5 overflow-hidden px-1 md:justify-center">{board.hands[0].map((tile, index) => <TileCard key={tile.id} tile={tile} onClick={() => discard(index)} disabled={!isHost || role !== "player" || board.gameMode !== "PLAYING" || board.phase !== "player" || board.turn !== 0 || isProcessingCpu} className={`hand-tile ${board.hands[0].length === 14 && index === 13 ? "ml-3" : ""}`} />)}</div><form onSubmit={sendChat} className="mt-2 flex gap-2 border-t border-amber-200 pt-2 md:hidden"><input value={chatText} onChange={(event) => setChatText(event.target.value)} placeholder="メッセージを入力" className="chat-input" /><button type="submit" className="send-button" aria-label="送信">送信</button></form></div>
        {isChatOpen && <div className="chat-history-modal fixed inset-0 z-[60] flex items-end justify-center md:hidden"><div className="chat-history-sheet"><div className="flex items-center justify-between border-b border-amber-200 pb-3"><h2 className="section-title">指導チャットの履歴</h2><button type="button" onClick={() => setIsChatOpen(false)} className="chat-close-button">閉じる</button></div><div className="chat-history-list">{messages.length === 0 ? <p className="py-8 text-center text-sm text-slate-400">まだメッセージはありません</p> : messages.map((message) => <div key={message.id} className={`chat-bubble ${message.role === role ? "mine" : ""}`}><div className="flex justify-between gap-2 text-[11px] font-bold text-slate-500"><span>{message.role === "coach" ? "指導者" : "打者"}</span><time>{message.time}</time></div><p className="mt-1 text-sm">{message.text}</p></div>)}</div></div></div>}
      </div>
    </main>
  );
}